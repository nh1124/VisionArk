...
import { getFileToken, BASE_URL } from "../lib/api";
                ...
                components={{
                    img({ node, src, alt, ...props }: any) {
                        ...
                        if (src && !src.startsWith("http")) {
                            // Clean up leading slashes for our check
                            const cleanSrc = src.startsWith("/") ? src.substring(1) : src;

                            if (cleanSrc.startsWith("artifacts/") || cleanSrc.startsWith("refs/") || cleanSrc.startsWith("files/")) {
                                processedSrc = `${BASE_URL}/api/files/${nodeType}/${nodeName}/${cleanSrc}${tokenQuery}`;
                            } else if (!cleanSrc.includes("/")) {
                                // Default simple filename to artifacts directory
                                processedSrc = `${BASE_URL}/api/files/${nodeType}/${nodeName}/artifacts/${cleanSrc}${tokenQuery}`;
                            } else {
                                // Fallback for other relative paths
                                processedSrc = `${BASE_URL}/${cleanSrc}${tokenQuery}`;
                            }
                        ...
                        const shouldShowCaption = alt && !isPathAlt && alt !== "Image";

                        console.log(`[MarkdownRenderer] Resolving Image. Original: ${src} -> Processed: ${processedSrc}`);
