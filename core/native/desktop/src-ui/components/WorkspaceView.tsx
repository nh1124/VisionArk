...
import {
    ...
    Upload, Download, FolderPlus, Eye, EyeOff, GripVertical, Maximize2, Minimize2,
} from "lucide-react"
...
type UploadModal = { file: globalThis.File; path: string; title: string; scope: Scope } | null
    ...
    return (
        <div className="flex-1 overflow-y-auto px-6 py-6 bg-[#030712] custom-scrollbar">
            <div className="max-w-6xl mx-auto h-[90vh] flex flex-col border border-gray-800/60 rounded-3xl overflow-hidden shadow-2xl">
                    ...
                    <div className="flex-1 overflow-y-auto bg-gray-950/20 custom-scrollbar">
                        ...
                        {!multiSelected && (selectedItem || isCreating) && (
                            <div className="flex flex-col h-full p-6 gap-4">
                                ...
                                {selectedItem?.item_type === "file" && (
                                    <>
                                        <div className="bg-gray-900/60 rounded-xl border border-gray-800/80 px-4 py-3 flex items-center gap-6 text-xs flex-shrink-0 mb-4">
                                        ...
                                        <div className={`flex flex-col ${expandedPreview ? "fixed inset-4 z-50 bg-gray-900 rounded-2xl border border-gray-700 shadow-2xl p-6" : "flex-1 min-h-[300px]"}`}>
                                            <div className="flex items-center justify-between mb-1.5 flex-shrink-0">
                                                <label className="text-[10px] font-bold text-gray-500 uppercase tracking-widest">Preview</label>
                                                <div className="flex items-center gap-2">
                                                    <button onClick={() => setExpandedPreview(v => !v)} className="flex items-center gap-1 text-[10px] text-gray-600 hover:text-gray-400 transition-colors">
                                            ...
                                            {showPreview && (
                                                <div className="flex-1 min-h-0 rounded-xl border border-gray-800 overflow-hidden bg-gray-950">
                                                    ...
                                                    {preview?.status === "text" && (
                                                        <pre className="h-full overflow-auto p-4 text-xs text-gray-300 font-mono leading-relaxed whitespace-pre-wrap break-words custom-scrollbar">{preview.content}</pre>
                                                    )}
                                ...
                                {(isCreating || selectedItem?.item_type === "note") && (
                                    <div className="flex flex-col flex-1 min-h-[300px]">
                                        <label className="block text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-1.5 flex-shrink-0">Content (Markdown)</label>
                                        <textarea placeholder="Write your content here…" value={form.content} onChange={e => patchForm({ content: e.target.value })}
                                            className="flex-1 w-full bg-gray-900 border border-gray-800 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-cyan-500/50 resize-none font-mono text-sm leading-relaxed custom-scrollbar min-h-0" />
                                    </div>
                                )}

                                {/* tags - moved below content/preview */}
                    ...
                    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                        <div className="bg-gray-900 border border-red-900/40 w-full max-w-sm rounded-2xl shadow-2xl p-6 animate-in fade-in zoom-in duration-150">
                            ...
                            <div className="flex gap-3 justify-end">
                                ...
                                <button onClick={handleBulkDelete} className="px-5 py-1.5 rounded-lg text-sm bg-red-500 hover:bg-red-400 text-white font-bold">Delete All</button>
                            </div>
                        </div>
                    </div>
                )}
                ...
                {uploadModal && (
                    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                        <div className="bg-gray-900 border border-gray-800 w-full max-w-sm rounded-2xl shadow-2xl p-6">
                            <h3 className="text-base font-bold text-white mb-4">Upload File</h3>

                            <div className="mb-4">
                                <label className="block text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-1.5">File</label>
                                <p className="text-sm font-mono text-gray-300 truncate">{uploadModal.file.name}</p>
                            </div>

                            <div className="mb-4">
                                <label className="block text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-1.5">Destination Path</label>
                                <input type="text" value={uploadModal.path} onChange={e => setUploadModal({ ...uploadModal, path: e.target.value })}
                                    className="w-full bg-black/40 border border-gray-800 rounded-xl px-3 py-2 text-white text-sm font-mono focus:border-cyan-500/50" />
                            </div>

                            <div className="mb-6">
                                <label className="block text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-1.5">Title (Optional)</label>
                                <input type="text" value={uploadModal.title} onChange={e => setUploadModal({ ...uploadModal, title: e.target.value })}
                                    className="w-full bg-black/40 border border-gray-800 rounded-xl px-3 py-2 text-white text-sm focus:border-cyan-500/50" />
                            </div>

                            <div className="flex gap-3 justify-end">
                                <button onClick={() => setUploadModal(null)} disabled={uploading} className="px-4 py-1.5 rounded-lg text-sm text-gray-400 hover:text-white disabled:opacity-40">Cancel</button>
                                <button onClick={handleUpload} disabled={uploading || !uploadModal.path} className="flex items-center gap-2 px-5 py-1.5 rounded-lg text-sm bg-cyan-500 hover:bg-cyan-400 text-black font-bold disabled:opacity-40 disabled:cursor-not-allowed">
                ...
                {folderModal && (
                    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                        <div className="bg-gray-900 border border-gray-800 w-full max-w-sm rounded-2xl shadow-2xl p-6">
                            ...
                            <div className="mb-4">
                                <label className="block text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-1.5">Parent Path</label>
                                <p className="text-sm font-mono text-gray-500 truncate">{currentPath || "/"}</p>
                            </div>

                            <div className="mb-6">
                                <label className="block text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-1.5">Folder Name</label>
                                <input type="text" autoFocus placeholder="e.g. assets" value={folderModal.name} onChange={e => setFolderModal({ ...folderModal, name: e.target.value })}
                                    className="w-full bg-black/40 border border-gray-800 rounded-xl px-3 py-2 text-white font-mono text-sm focus:border-cyan-500/50 outline-none"
                                    onKeyDown={e => { if (e.key === "Enter") handleCreateFolder() }} />
                            </div>

                            <div className="flex gap-3 justify-end">
                                <button onClick={() => setFolderModal(null)} disabled={creatingFolder} className="px-4 py-1.5 rounded-lg text-sm text-gray-400 hover:text-white disabled:opacity-40">Cancel</button>
                                <button onClick={handleCreateFolder} disabled={creatingFolder || !folderModal.name.trim()} className="flex items-center gap-2 px-5 py-1.5 rounded-lg text-sm bg-cyan-500 hover:bg-cyan-400 text-black font-bold disabled:opacity-40 disabled:cursor-not-allowed">
                                    {creatingFolder && <Loader2 size={14} className="animate-spin" />}
                                    Create
                                </button>
                            </div>
                        </div>
