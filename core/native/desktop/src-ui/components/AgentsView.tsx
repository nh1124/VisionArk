...
import { Bot, Plus, Pencil, Trash2, X, GitBranch, Check, Loader2 } from "lucide-react"
...
export default function AgentsView() {
    ...
    const handleSave = async (data: { display_name: string; description: string; skill_ids: string[]; graph_id: null }) => {
        if (editingAgent) {
                ...
                <div className="flex items-center justify-between max-w-7xl mx-auto">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-gray-400 text-sm mt-0.5">Manage reusable agents across all your projects</p>
                        </div>
                    </div>
                    <button
                                ...
                                <div key={agent.id} className="bg-gray-900 border border-gray-800 rounded-2xl p-5 flex flex-col gap-3 hover:border-gray-700 transition-colors">
                                    ...
                                    <div className="flex items-center gap-1.5">
                                        ...
                                        {agent.graph_id && agent.graph_id !== "direct_assistant" && (
                                            <span className="text-[9px] text-amber-500/70 border border-amber-500/20 rounded px-1">stored · pinned in engine</span>
                                        )}
                                    </div>
            ...
            {deleteConfirm && (
                <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                    <div className="bg-gray-900 border border-red-900/40 w-full max-w-sm rounded-2xl shadow-2xl p-6">
                        ...
                        <p className="text-gray-400 text-sm mb-5 font-mono">{deleteConfirm.display_name}</p>
                        <div className="flex gap-3 justify-end">
                            ...
                            <button onClick={() => handleDelete(deleteConfirm)} className="px-5 py-1.5 rounded-lg text-sm bg-red-500 hover:bg-red-400 text-white font-bold">Delete</button>
                        </div>
                    </div>
                </div>
            )}
