    ...
    async def load_system_prompt(self, role_name: Optional[str] = None, components: Optional[List[str]] = None) -> str:
        ...
        if node_id:
            try:
                ...
                async with AsyncSessionLocal() as db:
                    skill_text = await SkillService.inject_skills_to_prompt(
                        db,
                        node_id,
                        "",
        ...
        try:
            ...
            if node_id:
                try:
                    ...
                    from services.skill_service import SkillService
                    async with AsyncSessionLocal() as db:
                        tool_policy = await SkillService.get_node_tool_policy(
                            db,
                            node_id,
                            intent=tool_context.get("intent") if tool_context else None
                        )
                except Exception as e:
