...
class GenerateImageTool(BaseTool):
    ...
    async def run(self, prompt: str, filename: Optional[str] = None, **kwargs) -> Any:
        ...
        try:
            ...
            if not image_data:
                return ToolResult(content="No image generated.", is_success=False)
            ...
            # Since Gemini File API doesn't support immediate upload of generated bytes without a loopback,
            # we just return the text confirmation for now, but we COULD upload it to Gemini File API if needed.
            return ToolResult(
                content=f"✅ Generated and saved image: `{rel_path}`",
                data={"filename": filename, "embed_path": rel_path, "path": rel_path}
            )
        except Exception as e:
            return ToolResult(content=f"Image generation failed: {e}", is_success=False)
...
class ExecuteCodeTool(BaseTool):
    ...
    async def run(self, prompt: str, **kwargs) -> Any:
        ...
        try:
            ...
            return ToolResult(content=resp.text or "No text output from code execution")
        except Exception as e:
            return ToolResult(content=f"Code execution failed: {e}", is_success=False)
