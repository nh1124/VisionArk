...
from infrastructure.llm.reasoning_engine import ReasoningEngine
...
class BaseNode(ABC):
    ...
    async def load_system_prompt(self, role_name: Optional[str] = None, components: Optional[List[str]] = None) -> str:
        ...
        if components:
            for comp in components:
                ...
                if comp == "project_plan":
                    ...
                    if p_id and u_id:
                        try:
                            from shared.paths import get_plan_path, get_plan_injection_prompt_path
        ...
        if node_id:
            try:
                from domains.automation.skill_service import SkillService
    ...
    async def on_enter(self):
        ...
        """
        from shared.database import get_async_engine, get_async_session_maker
        from sqlalchemy import select
    ...
    async def chat_with_tools(
        ...
        try:
            ...
            # Apply dynamic tool policies from domains.automation.skills (allow/deny/intent mapping)
            tool_policy = {}
            node_id = self.context.get("node_id")
            if node_id:
                try:
