...
CRON_TO_PRESET = {v: k for k, v in PRESET_TO_CRON.items()}
...
def normalize_rule(rule: str) -> str:
    text = (rule or "").strip()
    if not text:
        raise ValueError("recurring_rule is required")
    return PRESET_TO_CRON.get(text, text)


def denormalize_to_preset_if_exact(cron_rule: str) -> str:
    text = (cron_rule or "").strip()
    return CRON_TO_PRESET.get(text, text)
...
def next_run_at_utc(
    ...
    parsed = parse_cron(recurring_rule)
...
def validate_recurring_rule(recurring_rule: str, timezone_name: str = "UTC") -> tuple[str, str]:
    normalized = normalize_rule(recurring_rule)
