...
class RAGService:
    """High-level RAG operations for a Spoke"""
    
    def __init__(self, spoke_name: str, session: Optional[Session] = None):
        self.spoke_name = spoke_name
        self.session = session
        self.vector_store = get_vector_store(spoke_name)
        self.pdf_processor = PDFProcessor(chunk_size=800, chunk_overlap=150)
        self.refs_dir = get_spoke_dir(spoke_name) / "refs"
    ...
    def search(
        ...
        Args:
            ...
            filter_file: Optional filename filter
    ...
    def get_indexed_files(self) -> List[Dict]:
        ...
        result = self.session.execute(query, {"spoke_name": self.spoke_name})
    ...
    def remove_file(self, filename: str):
        ...
        # TODO: Implement removal from vector store
        # Currently ChromaDB doesn't have easy way to delete by metadata
        # Would need to track document IDs or rebuild index
        pass
    ...
    def _is_indexed(self, pdf_path: Path, file_hash: str) -> bool:
        ...
        query = text("""
            ...
            WHERE spoke_name = :spoke_name AND file_path = :file_path
        """)
    ...
    def _update_index_metadata(self, pdf_path: Path, file_hash: str, chunk_count: int):
        ...
        upsert_query = text("""
            INSERT INTO rag_metadata (spoke_name, file_name, file_path, file_hash, chunk_count, indexed_at)
            VALUES (:spoke_name, :file_name, :file_path, :file_hash, :chunk_count, :indexed_at)
            ON CONFLICT(spoke_name, file_path) DO UPDATE SET
                file_hash = :file_hash,
