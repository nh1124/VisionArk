...
class KnowledgeCoreClient:
    ...
    async def _request(self, method: str, path: str, **kwargs) -> Dict[str, Any]:
        ...
        response = await self._async_client.request(method, path, headers=headers, **kwargs)
    ...
    async def login(self, email: str, password: str) -> TokenResponse:
        ...
        data = await self._request("POST", "/v1/auth/login", json={"email": email, "password": password})
        res = TokenResponse(**data)
        self.token = res.access_token
        self.api_key = None
        return res

    async def register(self, email: str, password: str, gemini_api_key: str, name: Optional[str] = None, is_admin: bool = False) -> Dict[str, Any]:
        """Register a new user."""
        return await self._request("POST", "/v1/auth/register", json={
            "email": email,
            "password": password,
    ...
    async def create_api_key(self, name: str, client_id: Optional[str] = None, scopes: List[str] = None, is_admin: bool = False) -> APIKeyNewResponse:
        ...
        data = await self._request("POST", "/v1/auth/keys", json=payload)
        return APIKeyNewResponse(**data)

    async def list_api_keys(self) -> List[APIKeyResponse]:
        """List active API keys for the current user."""
        data = await self._request("GET", "/v1/auth/keys")
        return [APIKeyResponse(**item) for item in data]

    async def revoke_api_key(self, key_id: Union[str, uuid.UUID]) -> Dict[str, str]:
        """Revoke an API key."""
        return await self._request("DELETE", f"/v1/auth/keys/{key_id}")

    async def get_my_profile(self) -> UserResponse:
        """Return current user profile (requires local Bearer token)."""
        data = await self._request("GET", "/v1/auth/me")
        return UserResponse(**data)

    async def get_my_key_info(self) -> APIKeyResponse:
        """Return current API key profile (requires X-API-KEY)."""
        data = await self._request("GET", "/v1/auth/keys/me")
        return APIKeyResponse(**data)

    async def change_password(self, current_password: str, new_password: str) -> Dict[str, str]:
        """Change the current user's password."""
        return await self._request("POST", "/v1/auth/password/change", json={
            "current_password": current_password,
            "new_password": new_password
        })

    async def update_user_settings(self, gemini_api_key: Optional[str] = None) -> Dict[str, str]:
        """Update per-user settings (Gemini API key)."""
        payload = {}
        if gemini_api_key is not None: payload["gemini_api_key"] = gemini_api_key
        return await self._request("PATCH", "/v1/auth/settings", json=payload)

    async def delete_account(self) -> Dict[str, Any]:
        """Delete the current user account and all associated data."""
        return await self._request("POST", "/v1/auth/delete-account")

    # --- System Administration ---

    async def get_system_config(self) -> SystemConfigResponse:
        """Fetch system configuration (Admin only)."""
        data = await self._request("GET", "/v1/auth/config")
        return SystemConfigResponse(**data)

    async def update_system_config(self, log_level: Optional[str] = None, debug: Optional[bool] = None) -> Dict[str, str]:
        """Update system configuration (Admin only)."""
        payload = {}
        if log_level is not None: payload["log_level"] = log_level
        if debug is not None: payload["debug"] = debug
        return await self._request("POST", "/v1/auth/config", json=payload)

    async def get_db_triggers(self) -> List[Dict[str, str]]:
        """Debug endpoint to list all triggers in the DB."""
        return await self._request("GET", "/v1/auth/debug/triggers")

    # --- Ingestion ---

    async def ingest_text(self, text: str, source: str = "api", scope: str = "global", 
        ...
        data = await self._request("POST", "/v1/ingest", json=payload)
        return IngestResponse(**data)

    async def get_ingest_status(self, ingest_id: str) -> Dict[str, Any]:
        """Get status of an ingestion job."""
        return await self._request("GET", f"/v1/ingest/{ingest_id}")

    async def wait_for_ingestion(self, ingest_id: str, timeout: int = 60, interval: int = 2) -> Dict[str, Any]:
        """Poll the server until the ingestion job is completed or failed."""
        start_time = time.time()
        while time.time() - start_time < timeout:
            status = await self.get_ingest_status(ingest_id)
    ...
    async def create_memory(self, content: str, memory_type: str = "fact", importance: int = 3, 
    ...
    async def list_memories(self, memory_type: Optional[str] = None, tags: Optional[str] = None, 
    ...
    async def get_memory(self, memory_id: Union[str, uuid.UUID]) -> MemoryResponse:
        """Get a single memory by ID."""
        data = await self._request("GET", f"/v1/memories/{memory_id}")
        return MemoryResponse(**data)

    async def update_memory(self, memory_id: Union[str, uuid.UUID], content: Optional[str] = None, 
        ...
        data = await self._request("PATCH", f"/v1/memories/{memory_id}", json=payload)
        return MemoryResponse(**data)

    async def delete_memory(self, memory_id: Union[str, uuid.UUID], hard: bool = False) -> Dict[str, Any]:
        """Delete a memory."""
        return await self._request("DELETE", f"/v1/memories/{memory_id}", params={"hard": hard})

    async def dump_memories(self, format: str = "json") -> Dict[str, Any]:
        """Export all memories."""
        return await self._request("GET", "/v1/dump", params={"format": format})

    # --- Context ---

    async def get_context(self, query: str, k: int = 10, include_global: bool = True, 
