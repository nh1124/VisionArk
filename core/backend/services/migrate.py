...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "create":
        ...
        # Discover migrations
        migrations_dir = Path(__file__).parent / "migrations"
        print(f"Searching for migrations in: {migrations_dir}")
        for file in sorted(migrations_dir.glob("*.py")):
            if file.name == "__init__.py":
                continue
            
            print(f"Found migration file: {file.name}")
            # Dynamic import
            module_name = f"services.migrations.{file.stem}"
            ...
            for attr_name in dir(module):
                ...
                # More robust check that avoids module mismatch issues
                if (isinstance(attr, type) and 
                    attr_name.startswith("Migration") and 
                    attr_name != "Migration" and
                    hasattr(attr, "up") and 
                    hasattr(attr, "down")):
                    print(f"  Loaded migration class: {attr_name} (version: {getattr(attr, 'version', 'unknown')})")
                    migrations.append(attr())
                    found_in_file = True
            
            if not found_in_file:
                print(f"  Warning: No Migration class found in {file.name}")
        
        print(f"Total migrations discovered: {len(migrations)}")
