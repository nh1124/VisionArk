...
export default function CronTasksPage() {
    ...
    const loadTasks = async () => {
        setLoading(true);
        try {
            // Fetch tasks with exclude_system=true by default as requested
    ...
    const loadProjects = async () => {
        try {
            const response = await apiFetch("/api/projects"); // Assuming a listing endpoint exists
            if (response.ok) {
                const data = await response.json();
    ...
    const handleEdit = (task: ScheduledTask) => {
        // Only allow editing POST_MESSAGE for now
            ...
            <div className="grid grid-cols-1 gap-6">
                {loading && tasks.length === 0 ? (
                    ...
                    <div className="py-20 flex flex-col items-center justify-center text-gray-600 bg-gray-900/20 rounded-3xl border border-dashed border-gray-800">
                        ...
                        <p className="font-bold uppercase tracking-widest text-xs">No active user tasks found</p>
                    </div>
                ) : (
                        ...
                        <table className="w-full text-left">
                            <thead className="bg-gray-800/50">
                                <tr>
                                    ...
                                    <th className="px-6 py-4 text-[10px] font-bold text-gray-500 uppercase tracking-widest">Last Run</th>
                                    ...
                                    <tr key={task.id} className="hover:bg-gray-800/20 transition-colors">
                                        <td className="px-6 py-4">
                                            <div className="flex flex-col">
                                                <span className="text-sm font-bold text-cyan-400">{task.task_type}</span>
                                                <span className="text-[10px] text-gray-600 font-mono truncate max-w-[100px]" title={task.id}>ID: ...{task.id.slice(-8)}</span>
                                            </div>
                                        </td>
                                        <td className="px-6 py-4">
