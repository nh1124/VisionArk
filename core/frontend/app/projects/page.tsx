...
export default function ProjectsPage() {
    ...
    const toggleSelectAll = (filteredProjects: Project[]) => {
        if (selectedProjects.size === filteredProjects.length && filteredProjects.length > 0) {
            ...
            setSelectedProjects(new Set(filteredProjects.map(s => s.name)));
        }
    };

    const cloneProject = async (name: string) => {
        // As per user feedback, start cloning with default name directly
        const newName = `${name}_copy`;

        try {
            const response = await apiFetch(`/api/agents/project/${name}/clone`, {
                method: "POST",
                headers: { "Content-Type": "application/json" },
    ...
    const deleteProject = async (name: string) => {
        ...
        try {
            await apiFetch(`/api/agents/project/${name}`, { method: "DELETE" });
            await loadProjects();
            setActiveMenu(null);
        } catch (error) {
        ...
        try {
            await Promise.all(
                Array.from(selectedProjects).map(name =>
                    apiFetch(`/api/agents/project/${name}`, { method: "DELETE" })
                )
            );
            setSelectedProjects(new Set());
            await loadProjects();
    ...
    const saveRename = async (projectName: string) => {
            ...
            {loading ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {filteredProjects.map((project) => {
                        ...
                        const isMenuOpen = activeMenu === project.name;
                                ...
                                <div className="flex items-start justify-between mb-5">
                                            ...
                                            {isRenaming ? (
                                                <div className="flex items-center gap-2 mt-1">
                                                    <input
                                                        ...
                                                        autoFocus
                                                        onKeyDown={(e) => {
                                                            if (e.key === "Enter") saveRename(project.name);
                                                            if (e.key === "Escape") setRenamingProject(null);
                                                        }}
                                                    />
                                                    <button onClick={() => saveRename(project.name)} disabled={savingRename} className="text-green-500 hover:text-green-400">
                                                        <Check size={18} />
                                                    </button>
                                    ...
                                    <div className="relative flex-shrink-0 ml-2">
                                        <button
                                            onClick={(e) => {
                                                e.stopPropagation();
                                                setActiveMenu(isMenuOpen ? null : project.name);
                                            }}
                                        ...
                                        {isMenuOpen && (
                                            ...
                                            >
                                                ...
                                                <button
                                                    onClick={() => cloneProject(project.name)}
                                                    className="w-full flex items-center gap-3 px-4 py-2 text-sm text-gray-300 hover:bg-cyan-500/10 hover:text-cyan-400 transition-colors"
                                                >
