...
import {
    Bot, Plus, Pencil, Trash2, X, GitBranch, Check,
...
function AgentModal({
    ...
    skills: Skill[];
    onClose: () => void;
    onSave: (data: { display_name: string; description: string; skill_ids: string[]; graph_id: string | null }) => Promise<void>;
}) {
                ...
                <form onSubmit={handleSubmit} className="p-5 space-y-4 overflow-y-auto">
                    ...
                    <div>
                        <div className="flex items-center gap-1.5 mb-1">
                            ...
                            <Tooltip text="グラフはエージェントの実行フローを決定します。現在は direct_assistant のみ対応しており、将来的にカスタムグラフを選択できるようになる予定です。" position="right" />
                    ...
                    <div>
                        <div className="flex items-center gap-1.5 mb-2">
                            ...
                            <Tooltip text="スキルはエージェントが使用できるツールセットを定義します。何も選択しない場合、すべてのスキルが有効になります。" position="right" />
                        </div>
...
function ModuleModal({
    ...
    // "pick" = show drop zone (create only); "edit" = show file editor
                    ...
                    <div className="p-6 flex flex-col gap-4">
                        ...
                        <p className="text-center text-xs text-gray-600">
                            ...
                            <button
                                type="button"
                                onClick={() => {
                                    setFiles({ "__init__.py": DEFAULT_INIT });
                                    setPhase("edit");
                                }}
                        ...
                        <div className="p-5 space-y-4 flex-1 min-h-0 overflow-y-auto">
                            ...
                            <div>
                                    ...
                                    {fileList.map(name => (
                                        <div key={name} className="flex items-center">
                                            <button type="button" onClick={() => setActiveFile(name)}
                                                className={`px-3 py-1 text-xs rounded-t-lg font-mono transition-colors ${activeFile === name ? "bg-gray-800 text-cyan-400 border border-b-0 border-gray-700" : "text-gray-500 hover:text-gray-300 bg-gray-900"}`}>
                                                {name}
                                            </button>
                                            {name !== "__init__.py" && (
                                                <button type="button" onClick={() => handleDeleteFile(name)}
                                    ...
                                    {addingFile ? (
                                        <div className="flex items-center gap-1">
                                            <input autoFocus value={newFileName} onChange={e => setNewFileName(e.target.value)}
                                                ...
                                                className="px-2 py-1 text-xs bg-gray-800 border border-cyan-500/50 rounded text-white font-mono w-32 outline-none"
                                        ...
                                        <button type="button" onClick={() => setAddingFile(true)}
                                            className="px-2 py-1 text-xs text-gray-500 hover:text-cyan-400 transition-colors flex items-center gap-1">
                                ...
                                <textarea
                                    ...
                                    className="w-full h-64 bg-gray-800 border border-gray-700 rounded-b-lg rounded-tr-lg px-3 py-3 text-white text-xs font-mono focus:outline-none focus:border-cyan-500/50 transition-colors resize-none"
                                    spellCheck={false}
                                />
                            </div>
                        </div>
            ...
            <div className="flex items-center justify-between p-4">
                ...
                <div className="flex items-center gap-1 flex-shrink-0 ml-2">
                    <span className="text-[10px] text-gray-600 mr-2">
                        {module.tools.length}T · {module.skills.length}S
                    </span>
                    <button onClick={() => onEdit(module)}
                        className="p-1.5 text-gray-500 hover:text-cyan-400 hover:bg-gray-800 rounded-lg transition-colors" title="Edit">
                        <Pencil size={13} />
                    </button>
                    <button onClick={() => onDelete(module.module_name)}
                        className="p-1.5 text-gray-500 hover:text-red-400 hover:bg-gray-800 rounded-lg transition-colors" title="Delete">
                        <Trash2 size={13} />
                    </button>
                </div>
            </div>
                                    ...
                                    <div key={skill.name} className="flex items-center justify-between gap-2">
                                        <div className="min-w-0">
                                            ...
                                            {skill.tools.length > 0 && (
                                                <span className="text-[10px] text-gray-600 ml-2">[{skill.tools.join(", ")}]</span>
                                            )}
                                        </div>
            ...
            <div className="bg-gray-900 border border-gray-700 rounded-2xl w-full max-w-xl mx-4 shadow-2xl max-h-[90vh] flex flex-col">
                ...
                <form onSubmit={handleSubmit} className="p-5 flex flex-col gap-4 overflow-y-auto flex-1">
                    ...
                    <div>
                        ...
                        <p className="text-[10px] text-gray-600 mt-1">Supports .yaml/.yml (single or multi-skill) or .md (YAML frontmatter + Markdown body as instructions)</p>
                    </div>
                    {error && (
                        <div className="flex items-center gap-2 text-red-400 text-sm bg-red-500/10 border border-red-500/20 rounded-lg px-3 py-2">
                            <AlertCircle size={14} />{error}
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-2xl overflow-hidden hover:border-gray-700 transition-colors">
            <div className="flex items-center justify-between p-4">
                ...
                <div className="flex items-center gap-1 flex-shrink-0 ml-2">
                    ...
                    <button onClick={() => onEdit(pack)} className="p-1.5 text-gray-500 hover:text-cyan-400 hover:bg-gray-800 rounded-lg transition-colors" title="Edit"><Pencil size={13} /></button>
                    <button onClick={() => onDelete(pack.pack_name)} className="p-1.5 text-gray-500 hover:text-red-400 hover:bg-gray-800 rounded-lg transition-colors" title="Delete"><Trash2 size={13} /></button>
                </div>
            </div>
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-2xl overflow-hidden hover:border-gray-700 transition-colors">
            <div className="flex items-center justify-between p-4">
                ...
                <div className="flex items-center gap-1 flex-shrink-0 ml-2">
                    ...
                    <button onClick={handleSync} disabled={syncing} title="Sync tools"
                        className="p-1.5 text-gray-500 hover:text-cyan-400 hover:bg-gray-800 rounded-lg transition-colors disabled:opacity-50">
                        <RotateCcw size={13} className={syncing ? "animate-spin" : ""} />
                    </button>
                    <button onClick={() => onEdit(server)} className="p-1.5 text-gray-500 hover:text-cyan-400 hover:bg-gray-800 rounded-lg transition-colors" title="Edit"><Pencil size={13} /></button>
                    <button onClick={() => onDelete(server.name)} className="p-1.5 text-gray-500 hover:text-red-400 hover:bg-gray-800 rounded-lg transition-colors" title="Delete"><Trash2 size={13} /></button>
                </div>
            </div>
            {expanded && (
                <div className="border-t border-gray-800 px-4 pb-4 pt-3 space-y-3">
                    ...
                    {server.last_synced_at && (
                        <p className="text-[10px] text-gray-600">Synced: {new Date(server.last_synced_at).toLocaleString()}</p>
                    )}
                    {server.sync_error && (
                    ...
                    {!server.last_synced_at && (
                        <p className="text-xs text-gray-600 italic">Not yet synced — click the sync button to load tools.</p>
                    )}
                </div>
            )}
        </div>
...
export default function AgentsPage() {
    ...
    const handleSaveAgent = async (data: { display_name: string; description: string; skill_ids: string[]; graph_id: string | null }) => {
        if (editingAgent) {
    ...
    const handleSaveModule = async (moduleName: string, files: Record<string, string>) => {
        if (editingModule) {
            ...
            const res = await apiFetch("/api/definitions/modules", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ module_name: moduleName, files }),
            });
            if (!res.ok) {
                const err = await res.json();
        ...
        await loadModules();
        // Reload skills list (new module may add skills)
        await loadAgents();
    };
    ...
    const handleToggleTool = async (moduleName: string, toolName: string, active: boolean) => {
        ...
        setModules(prev => prev.map(m =>
            m.module_name === moduleName
                ? { ...m, tools: m.tools.map(t => t.name === toolName ? { ...t, is_active: active } : t) }
                : m
        ));
    };
    ...
    const handleToggleSkill = async (moduleName: string, skillName: string, active: boolean) => {
        ...
        setModules(prev => prev.map(m =>
            m.module_name === moduleName
                ? { ...m, skills: m.skills.map(s => s.name === skillName ? { ...s, is_active: active } : s) }
                : m
        ));
    };
    ...
    const loadSkillPacks = async () => {
        ...
        try {
            ...
            console.error("Failed to load skill packs:", err);
        } finally {
    ...
    const handleSavePack = async (packName: string, filename: string, content: string) => {
        if (editingPack) {
            ...
            const res = await apiFetch("/api/definitions/skill-packs", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ pack_name: packName, filename, content }),
            });
    ...
    const handleDeletePack = async (name: string) => {
        try {
            ...
            console.error("Failed to delete skill pack:", err);
        } finally {
    ...
    const handleTogglePackSkill = async (packName: string, skillName: string, active: boolean) => {
        ...
        setSkillPacks(prev => prev.map(p =>
            p.pack_name === packName
                ? { ...p, skills: p.skills.map(s => s.name === skillName ? { ...s, is_active: active } : s) }
                : p
        ));
    };
    ...
    const handleSyncMcp = async (name: string) => {
        const res = await apiFetch(`/api/definitions/mcp/servers/${name}/sync`, { method: "POST" });
        await loadMcpServers();
        // Reload expanded server detail if needed
        if (!res.ok) {
    ...
    return (
        <div className="h-screen flex flex-col bg-gray-950">
            ...
            <div className="bg-gray-900 border-b border-gray-800 p-6">
                ...
                {/* Tab switcher */}
                <div className="flex gap-1 mt-4 border-b border-gray-800 -mb-6">
                    {([
                                ...
                                <div key={agent.id} className="bg-gray-900 border border-gray-800 rounded-2xl p-5 flex flex-col gap-3 hover:border-gray-700 transition-colors">
                                    <div className="flex items-start justify-between gap-2">
                                        ...
                                        <div className="flex items-center gap-1 flex-shrink-0">
                                            <button onClick={() => { setEditingAgent(agent); setAgentModalOpen(true); }}
                                                className="p-1.5 text-gray-500 hover:text-cyan-400 hover:bg-gray-800 rounded-lg transition-colors" title="Edit">
                                                <Pencil size={14} />
                                            </button>
                                            <button onClick={() => handleDeleteAgent(agent.id)} disabled={deletingAgentId === agent.id}
                                                className="p-1.5 text-gray-500 hover:text-red-400 hover:bg-gray-800 rounded-lg transition-colors" title="Delete">
                                                <Trash2 size={14} />
                                            </button>
                                        </div>
                                    ...
                                    {agent.skill_ids.length > 0 ? (
                                        <div className="flex flex-wrap gap-1.5">
                                            {agent.skill_ids.map(s => (
                                                <span key={s} className="px-2 py-0.5 bg-cyan-500/10 text-cyan-400 border border-cyan-500/20 rounded-full text-xs font-medium">{s}</span>
                                            ))}
                                        </div>
                                    ) : (
                        ...
                        <div className="max-w-3xl mx-auto space-y-3">
                            ...
                            {mcpServers.map(s => (
                                <MCPServerCard key={s.name} server={s}
                                    ...
                                    onSync={async name => { await handleSyncMcp(name); await loadMcpServerDetail(name); }}
