...
2. Call `codex_run` with a clear natural-language `prompt` describing the coding task.
   ...
   - Relative paths (including `"."`) are accepted and resolved on the device side.
   - Prefer explicit absolute project paths for stable/reproducible runs; treat `"."` as convenience/debug usage, not recommended for production automation.
...
6. If a job remains `running` with unchanged output for too long, stale protection can end it automatically.
   - This is returned as `failed / stalled`.
   - Re-run with an explicit absolute `workdir` and inspect `codex_job_output` for the last visible console state.
...
- `workdir` - Working directory on the device (optional; strongly recommended for file tasks)
  - Supports absolute and relative paths. `"."` is allowed, but absolute paths are preferred.
...
- `failed / stalled` - Output/status did not progress for a configured stale window
